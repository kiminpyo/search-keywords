...
const Test = (): JSX.Element => {
  ...
  const searchItems = keywordList?.map((item: any, idx) => {
    const startKeywordItem: number = item.sickNm.indexOf(inputValue);
    const boldText: string[] = item.sickNm.split(inputValue);
    return (
      <SearchItem
        rawData={item.sickNm}
        key={idx}
  ...
  if (inputValue === '') {
    ...
    return (
      <div>
        <input type="text" value={inputValue} onChange={onChange} ref={ref} />
        <div>{inputValue}</div>
